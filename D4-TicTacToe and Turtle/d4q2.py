...
def joue (tab, joueur):
    ...
    while not valid:   
        ...
        while not((0 <= place[0] < len(tab)) and (0 <= place[1] < len(tab))):
          # verifie que les nombres choisi sont dans le tableau
          # demande l'usager pour les coordonnées
        ...
        if tab[place[0]][place[1]] != '-':
          # si la position est occupée, retourner au début du boucle
          print("La position", place[0], place[1], "est occupée");
          valid = False
        else:
          valid = True             
          # met la position du joueur dans la matrice 
